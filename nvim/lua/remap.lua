...
function Insert.put(text, no_space)
	...
	local prefix = no_space and "" or " "
	vim.api.nvim_buf_set_text(0, row - 1, col, row - 1, col, { prefix .. text })
end

function Insert.date(no_space)
	Insert.put(os.date("%Y-%m-%d"), no_space)
end

function Insert.timestamp(no_space)
...
end, { desc = "Insert timestamp (with space)" })

vim.keymap.set("n", "<leader>tD", function()
	Insert.date(true)
end, { desc = "Insert date (no space)" })

vim.keymap.set("n", "<leader>tS", function()
	Insert.timestamp(true)
end, { desc = "Insert timestamp (no space)" })
